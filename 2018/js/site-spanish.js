	...
	var methods = {
		...
		showResults1: function() {
			...
			if ( methods.plans.depends >= topTally || methods.plans.hra == methods.plans.hsa ) {
				...
				if (methods.plans.hsa > methods.plans.hra) {
					$('.plan').css('font-size', font_size).html('Depende, pero considere el plan HSA.');
					if ( methods.windowWidth <= methods.defaults.breakpoint ) {
						$('.plan').css('font-size', '110%').html('Depende');
					}
				} else {
					$('.plan').css('font-size', font_size).html('Depende, pero considere el plan HRA.');
					if ( methods.windowWidth <= methods.defaults.breakpoint ) {
						$('.plan').css('font-size', '110%').html('Depende');
					}
				}
